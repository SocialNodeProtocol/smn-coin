...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = "xauto" && test x$bitcoin_qt_force != xyes; then
    if test x$bitcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; socialnode-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build socialnode-qt GUI (default=auto, qt5 tried first)])],
    [
...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  BITCOIN_QT_CHECK([
  TSMN_CPPFLAGS=$CPPFLAGS
  TSMN_CXXFLAGS=$CXXFLAGS
  CPPFLAGS="$QT_INCLUDES $CPPFLAGS"
  CXXFLAGS="$PIC_FLAGS $CXXFLAGS"
  if test x$bitcoin_qt_got_major_vers = x5; then
    TSMN_CPPFLAGS="$TSMN_CPPFLAGS -DHAVE_QT5"
  ...
  CPPFLAGS=$TSMN_CPPFLAGS
  CXXFLAGS=$TSMN_CXXFLAGS
  ])
...
AC_DEFUN([_BITCOIN_QT_CHECK_STATIC_PLUGINS],[
  ...
  CHECK_STATIC_PLUGINS_TSMN_LIBS="$LIBS"
  LIBS="$2 $QT_LIBS $LIBS"
  ...
  LIBS="$CHECK_STATIC_PLUGINS_TSMN_LIBS"
])
