...
#include "ui_zsocialnodecontroldialog.h"
...
std::list<std::string> ZSocialNodeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSocialNodeControlDialog::listMints;

ZSocialNodeControlDialog::ZSocialNodeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSocialNodeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSocialNodeControlDialog::~ZSocialNodeControlDialog()
{
    delete ui;
}

void ZSocialNodeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSocialNodeControlDialog::updateList()
{
...
void ZSocialNodeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSocialNodeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSocialNode_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSocialNodeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSocialNodeControlDialog::GetSelectedMints()
{
...
void ZSocialNodeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
