...
#include "zsocialnodecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSMN ought to be enough for anybody." - Bill Gates, 2017
    ui->zSMNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSMNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSMN->setEnabled(false);
        ui->pushButtonMintzSMN->setToolTip(tr("zSMN is currently disabled due to maintenance."));

        ui->pushButtonSpendzSMN->setEnabled(false);
        ui->pushButtonSpendzSMN->setToolTip(tr("zSMN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSMNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSMN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSMN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSMN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSMN in ") +
...
void PrivacyDialog::on_pushButtonSpendzSMN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSMN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSMN
        sendzSMN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSMN
    sendzSMN();
}

void PrivacyDialog::on_pushButtonZSocialNodeControl_clicked()
{
    ZSocialNodeControlDialog* zSMNControl = new ZSocialNodeControlDialog(this);
    zSMNControl->setModel(walletModel);
    zSMNControl->exec();
}

void PrivacyDialog::setZSocialNodeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSMNSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSMN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid SocialNode Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSMNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSMN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SocialNode </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSMN</b>";
    ...
    // use mints from zSMN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSocialNodeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSocialNodeControlDialog::GetSelectedMints();
    }
    ...
    // Clear zsocialnode selector in case it was used
    ZSocialNodeControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSMN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " SocialNode, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSMN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSMN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSMN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSMN "));
    ui->labelzSMNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSMNSyncStatus->setVisible(fShow);
}
