...
#if defined(NDEBUG)
#error "SocialNode cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSMN spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zSMN spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zSMN spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    nSubsidy = 1000 * COIN;    
      } else if (nHeight <= 32120 && nHeight > 100) {
    nSubsidy = 1 * COIN;
      } else if (nHeight <= 64240 && nHeight > 32120) {
    nSubsidy = 2 * COIN;
      } else if (nHeight <= 96360 && nHeight > 64240) {
    nSubsidy = 3 * COIN;
      } else if (nHeight <= 128480 && nHeight > 96360) {
    nSubsidy = 4 * COIN;
      } else if (nHeight <= 160600 && nHeight > 128480) {
    nSubsidy = 5 * COIN;
      } else if (nHeight <= 192720 && nHeight > 160600) {
    nSubsidy = 6 * COIN;
      } else if (nHeight <= 224840 && nHeight > 192720) {
    nSubsidy = 7 * COIN;
      } else if (nHeight <= 256960 && nHeight > 224840) {
    nSubsidy = 8 * COIN;
      } else if (nHeight <= 289080 && nHeight > 256960) {
    nSubsidy = 9 * COIN;
      } else {
    nSubsidy = 6 * COIN;
      }
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60) && chainActive.Height() > 100;
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SocialNode
...
{
    RenameThread("socialnode-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSocialNodeMinted()
{
...
void RecalculateZSocialNodeSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
        ...
        //Remove spends from zSMN supply
...
bool RecalculateSocialNodeSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zSMN with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSMNSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SocialNode to zSMN
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSMN spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSMN serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSMN mints and %d zSMN spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SocialNode: We use certain sporks during IBD, so check to see if they are
